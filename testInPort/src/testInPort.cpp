...
static const char* const testinport_spec[] =
    {
...
RTC::ReturnCode_t testInPort::onExecute(RTC::UniqueId ec_id)
{
  ...
  m_inIn.read();
  cv::Mat img(cv::Size(m_in.data.image.width, m_in.data.image.height), CV_8UC3, (void *)&(m_in.data.image.raw_data[0]));
  cv::imshow("CaptureImage", img);
